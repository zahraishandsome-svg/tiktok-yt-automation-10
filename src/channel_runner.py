        ...
        while True:
            ...
            if upload_mode == "dual":
                ...
                if slot == 2:
                    ...
                    _run_short_only(channel, video, slot, run_id, dry_run, result)
            elif upload_mode in ("popular_split", "popular_only"):
...
def _pick_next_video(channel: Dict[str, Any], slot: int,
    ...
    # popular_only: EVERY slot takes the most-VIEWED unposted TikTok, mining the
    # back-catalogue best-first. Once the catalogue is exhausted the only unposted
    # videos left are the new ones, so it degrades to "latest" on its own.
    if upload_mode == "popular_only":
        return _pick_most_popular(channel, exclude_ids=exclude_ids)
...
def _pick_most_popular(channel: Dict[str, Any],
    ...
    already_posted = db.get_posted_video_ids(channel_id, upload_mode=channel.get("upload_mode", "popular_split"))
